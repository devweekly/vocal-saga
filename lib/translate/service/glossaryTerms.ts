 ...
 * glossary 的净化。
 *
 * 只有一个职责：把 `document_terms`（需**保留原文**的专有名词，如公司/产品/服务名）
 * 清洗成可安全嵌入 prompt 的纯数据列表。
 *
 * 术语表是**无感**的：由 `glossaryExtractor` 从被翻译页面的正文里自动抽取，
 * 用户不需要参与、也不提供管理界面。它只为提升一点术语一致性，不是知识库。
 *
 * ## 威胁模型
 *
 * `document_terms` 的来源不可信 —— 它抽取自**被翻译页面的正文**，页面内容由站方控制。
 *
 ...
 *
 * 的文本并被抽成"术语"，就能闭合 `<术语表>` 标签、劫持整条翻译指令
 * （prompt injection）。
 *
 * ## 防御思路
 *
 ...
 *
 * 独立成文件的原因：prompt-contract.ts 会 import 本文件渲染术语表段落，
 * 若把本函数放进 prompt-contract.ts，各文风模块再 import 它就会形成运行时
 * 循环依赖。
 */
